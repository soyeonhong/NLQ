...
class ReferringRecall(object):
    ...
    def __init__(
            self,
            dataset="ego4d",
            gt_val_file="/remote-home/share/Ego4D_dsz/v1/annotations/nlq_val.json",
            gt_train_file="/data/datasets/ego4d_data/v2/annotations/nlq_train.json"
    ):
        self.dataset = dataset
        # self.gt_file = gt_file
        self.gt_val_file = gt_val_file
        self.gt_train_file = gt_train_file
        print(self.gt_val_file)
        print(self.gt_train_file)
        if self.dataset == "ego4d":
            with open(self.gt_val_file) as file_id:
                self.gt_val_dict, self.num_gt_val_queries = self.load_gt_from_json(json.load(file_id))
            with open(self.gt_train_file) as file_id:
                self.gt_train_dict, self.num_gt_train_queries = self.load_gt_from_json(json.load(file_id))
        else:
            self.gt_dict = {}
